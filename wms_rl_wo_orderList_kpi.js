...
define(['./wms_utility', './wms_translator', './big', './wms_workOrderUtility_kpi', 'N/query'],
	...
	function (utility, translator, Big, woUtility, query) {
		...
		function doPost(requestBody) {
			...
			// Iterate through the orderList array and log each internalId
			for (var i = 0; i < workOrdListResults.length; i++) {
				intId += workOrdListResults[i]['internalid'] + ', ';
			}

			log.debug('orderListDetails', 'internalId: ' + intId);

			/* for (var b = 0; b < backOrderedInternalIdArr.length; b++) {
				bckIntId += backOrderedInternalIdArr[b] + ', ';
			} */

			log.debug('backOrderListDetails', 'internalId: ' + backOrderedInternalIdArr);
