 ...
 * @NModuleScope Public
 */
define(['N/record', 'N/search', './wms_utility', './wms_translator', './wms_inventory_utility'],

    function (record, search, utility, translator, invtUtility) {
        ...
        function doPost(requestBody) {
            ...
            try {
                if (utility.isValueValid(requestBody)) {
                    ...
                    var curr_page = requestParams.curr_page;
                    var lotBinArr = requestParams.lotBinArr;
                    var submitting = requestParams.submitting;
                    var upccode = requestParams.upccode;
                                ...
                                if (objBinDetails != null && objBinDetails != '') {
                                    if (objBinDetails != undefined && objBinDetails != 'undefined') {
                                        ...
                                        if (useBins == false) {
                                            ...
                                            log.debug('submitting', submitting);

                                            if (submitting == 'true' && (!lotBinArr || lotBinArr['lotBinArray'].length === 0)) {

                                                log.debug('no bins flag', true);

                                                binoritemArray.errorMessage = 'Please enter inventory detail for this item.';
                                                binoritemArray.isValid = false;

                                            } else {

                                                binoritemArray = {};
                                                binoritemArray.curr_page = curr_page;
                                                ...
                                                binoritemArray.upccode = inputStringVar;
                                                ...
                                                log.debug('inputStringVar', inputStringVar);

                                                log.debug('upccode', upccode);

                                                log.debug('if cond 116 left hand', lotBinArr && lotBinArr['lotBinArray']);

                                                log.debug('if cond 116 right hand', inputStringVar != upccode);

                                                if ((lotBinArr && lotBinArr['lotBinArray']) && (inputStringVar != upccode)) {

                                                    log.debug('if cond 116', true);
        ...
        function binValidation(requestParams) {
            ...
            if (!utility.isValueValid(binInternalId)) {
                ...
                if (binSearchResults[0].getText({ name: 'custrecord_wmsse_bin_loc_type' }) == 'Stage' &&
                    ...
                    binoritemArray.binName = binSearchResults[0].getValue({ name: 'binnumber' });
